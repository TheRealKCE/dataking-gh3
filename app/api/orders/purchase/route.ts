...
import { sendMtnVerificationPendingSMS } from '@/lib/sms-service'
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { packageId, phoneNumber, referenceCode: clientReferenceCode, acknowledgeRegistration } = body
        ...
        // Must run before the wallet is touched: an unregistered number cannot receive
        // data, so charging first and only discovering it at fulfillment is the bug this
        // fixes. On the acknowledged re-submit this hits the local cache, not the supplier.
        ...
        // Acknowledged: the order is created but held, and auto-fulfillment is skipped —
        // the whitelist would reject that call anyway.
        const heldForRegistration = gate.gated
        ...
        // A held order is not "being processed" — say what is actually happening, or
        // the buyer chases support when nothing lands.
        const progressMessage = heldForRegistration
            ? `Your order for ${(pkg as any).size} to ${phoneNumber} is waiting for the number to be registered on MTN. It will be delivered automatically once that completes.`
            : `Your order for ${(pkg as any).size} to ${phoneNumber} has been placed and is being processed.`

        ;(supabase.from('notifications') as any).insert({
            user_id: userId,
            title: heldForRegistration ? 'Order Held — Awaiting Registration' : 'Order Placed',
        ...
        await sendPushToUser(userId, {
            title: heldForRegistration ? 'Order Held — Awaiting Registration' : 'Order Placed',
            body: heldForRegistration
                ? `${phoneNumber} is being registered on MTN. Your ${(pkg as any).size} bundle delivers automatically once it is ready.`
                : `Your ${(pkg as any).size} bundle for ${phoneNumber} is being processed.`,
        ...
        try {
            ...
            if (userFetchError || !userData) {
                ...
                // Held for registration: the whitelist would reject the supplier call, so
                // don't make it. Tell the recipient once, here — the self-heal cron
                // (agentportal-mtn-verify) picks the order up and delivers it later, and it
                // must NOT send this SMS on every retry.
                if (heldForRegistration) {
                    await sendMtnVerificationPendingSMS((order as any).phone_number, {
                        network: (pkg as any).network,
                        size: (pkg as any).size,
                    }).catch((err: Error) => console.error('[Purchase] Verification-pending SMS failed:', err))
                } else if ((pkg as any).network === 'EXPRESS MTN') {
