...
export async function GET(request: NextRequest) {
    try {
        ...
        // Filter for orders where api_response contains supplier: datagod
        // In Supabase we can use .contains on the inner join path
        query = query.contains('mtn_fulfillment_tracking.api_response', { supplier: 'datagod' })
        ...
        if (fetchError) {
            console.error('[DataGodHistoryFetch] Error:', fetchError)
            throw fetchError
        }

        const orders = (rawOrders || [])
            .filter((order: any) => {
                // If the only DataGod tracking logs are failed ones, exclude this order from History
                const hasSuccessfulDataGodAttempt = order.mtn_fulfillment_tracking?.some(
                    (t: any) => t.api_response?.supplier === 'datagod' && t.api_response?.note !== 'Manual DataGod Fulfillment Failed'
                )
                return hasSuccessfulDataGodAttempt
            })
            .map((order: any) => {
        ...
        console.error('DataGod History Orders Fetch Error:', error)
