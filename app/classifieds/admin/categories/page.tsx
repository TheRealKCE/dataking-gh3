...
// Stable, module-level row so it never remounts on parent state changes.
// The file picker is triggered by a native <label htmlFor> (reliable on mobile
// taps — a programmatic input.click() on a hidden input is flaky there).
function CategoryRow({
    ...
    sub?: boolean
    busy: boolean
    onUpload: (cat: ClassifiedCategory, file: File) => void
    onRemove: (cat: ClassifiedCategory) => void
}) {
    const inputId = `cat-file-${cat.id}`
    return (
        <div
        ...
        >
            ...
            <label
                htmlFor={inputId}
                aria-disabled={busy}
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer select-none ${busy ? 'opacity-50 pointer-events-none' : ''}`}
            >
