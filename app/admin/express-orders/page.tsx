...
export default function ExpressOrdersPage() {
    ...
    const fetchOrders = async (resetSelection = false) => {
        ...
        try {
            ...
            // Filter strictly for EXPRESS MTN orders
            const expressOrders = (data.orders || []).filter((o: Order) => o.network === 'EXPRESS MTN')
            setOrders(expressOrders)
        } catch (error: any) {
    ...
    return (
        <div className="px-2 py-4 md:p-8 max-w-[1400px] mx-auto space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs md:text-sm text-muted-foreground mt-1">
                        Manual fulfillment queue for EXPRESS MTN packages.
                    </p>
                </div>
            ...
            <Card className="bg-gradient-to-br from-orange-400 to-yellow-500 text-black border-none shadow-lg">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-xl md:text-2xl font-black">
                                {formatCurrency(filteredOrders.reduce((sum, o) => sum + o.price, 0))}
                            </p>
                        </div>
                    </div>
                </CardContent>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="lg:col-span-3 space-y-4">
                    {selectedOrders.size > 0 && (
                        <div className="sticky top-20 z-30 flex items-center justify-between bg-orange-50 dark:bg-orange-900/20 backdrop-blur-md border border-orange-200 dark:border-orange-900 p-2 md:p-3 rounded-xl shadow-lg animate-in fade-in flex-wrap gap-2 mx-2">
                            <div className="flex items-center gap-2">
                                <Badge variant="default" className="rounded-full px-2 text-[10px] bg-orange-500 text-black">{selectedOrders.size}</Badge>
                                <span className="text-[10px] md:text-xs font-bold uppercase text-orange-900 dark:text-orange-400">Selected</span>
                            </div>
                        ...
                        <CardContent className="px-1 md:px-6">
                            {filteredOrders.length === 0 && !isLoadingOrders ? (
                                <div className="text-center py-20 border-2 border-dashed rounded-xl m-2">
                                    <Zap className="w-12 h-12 mx-auto text-orange-400/30 mb-3" />
                                    <h3 className="text-sm font-bold">No Pending EXPRESS MTN Orders</h3>
                                        ...
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                selectedOrders.has(order.id)
                                                    ? "bg-orange-500/10 border-orange-500 shadow-md scale-[1.01]"
                                                    : "bg-card border-transparent hover:border-orange-500/20 hover:bg-accent/50"
                                            )}
                                        >
                                            <div
                                            ...
                                            <div className="flex-1 grid grid-cols-2 lg:grid-cols-5 items-center gap-x-2 gap-y-3 p-1">
                                                ...
                                                <div className="space-y-0.5">
                                                    ...
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="text-xs font-black bg-orange-500 text-black px-1 rounded">{order.size}</span>
                                                    </div>
                                                </div>
